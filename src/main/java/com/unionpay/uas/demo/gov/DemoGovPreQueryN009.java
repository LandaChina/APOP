...
public class DemoGovPreQueryN009 {

    private static final Logger logger = Logger.getLogger(DemoGovPreQueryN009.class);
...
//        RsaDemo();
    ...
    public static void GmDemo() {
        ...
        reqInfoJson.put("entrust_dt","20240409");//委托日期
        reqInfoJson.put("reg_num","20123700920000570357");//委托日期
        JSONObject reqList = new JSONObject();
        reqList.put("tax_auth","13701020000");
        reqList.put("ss_agen_cd","377102");
        reqList.put("per_num","12370102196211302519");
        reqList.put("col_proj_cd","10203");
        reqList.put("col_item_cd","102031201");
        reqList.put("col_sub_cd","1020312011371040");
        reqList.put("pay_base","4242.00");
        reqList.put("expi_dt","202404");
        JSONArray reqListA = new JSONArray();
        reqListA.put(reqList);
        reqInfoJson.put("req_list",reqListA);
        Map<String, Object> bodyMap = new HashMap<>();
        Map<String, Object> encDataMap = new HashMap<>(); // 需要加密的敏感信息报文 根据实际业务填写
        encDataMap.put("cert_num", "370102197602294522"); //证件号码
...
//        bodyMap.put(SDKConstants.param_encryptData, toBeEncMap); // 如果不需要携带敏感信息并由网关解密, 则注释这行代码
        ...
        bodyMap.put("bussCode", "S2_4500_N009"); //业务代码
