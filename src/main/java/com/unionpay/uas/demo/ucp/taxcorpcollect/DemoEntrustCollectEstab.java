...
/**
 * 内容网关税费对公代收业务，签约接口demo
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class DemoEntrustCollectEstab {

    private static final Logger logger = Logger.getLogger(DemoEntrustCollectEstab.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.tax.corporate.entrust.collect.establish");       // 交易类型 根据实际业务填写
        ...
        reqData.put(SDKConstants.param_appType, "02");                   // 00银联索引 01机构索引、02商户索引
        ...
        bodyMap.put("bussMerId", "987654321011111"); //不上送征收机关代码时，必填
        bodyMap.put("colOrganCd", "21100000000"); //不上送征收机关代码时，必填
        bodyMap.put("bussCode", "S0_9800_0000"); //业务代码
        bodyMap.put("bussInfo", "91310114MA1GXBNG2T"); //纳税人识别号
        bodyMap.put("refBizType", "09"); //09：委托收款

        //发起方账户信息域	fwdAcctInfo
        Map<String, Object> fwdAcctInfoMap = new HashMap<>();
        fwdAcctInfoMap.put("bankNo", "102100009997"); //开户行行号
        fwdAcctInfoMap.put("acctNo", "6222581234567897311212"); //账号
        fwdAcctInfoMap.put("acctName", "小王"); //账户名称
        //账号类型取值：x1：个人银行卡借记账户 x2：个人银行卡贷记账户 x3：个人银行非卡账户
        // x4：个人银行支付账户 y1：单位银行卡账 y2：单位银行非卡账户 y3：单位支付账户 99：其他
        fwdAcctInfoMap.put("acctType", "x1");

        // 签约账户信息域 contractAcctInfo
        Map<String, Object> contractAcctInfoMap = new HashMap<>();
        // 签约交易时必填字段 bankNo、acctNo、acctType、acctName
        contractAcctInfoMap.put("bankNo", "102100009997"); //开户行行号
        contractAcctInfoMap.put("acctNo", "6222581234567897311212"); //账号
        ...
        */
        ...
        // 对公委托收款信息域 agDeductInfo
        Map<String, Object> agDeductInfoMap = new HashMap<>();
        // 签约交易时必填字段 agReceiveSubs、agReceivePeriod、deductionType
        /* agReceiveSubs取值：
        通讯服务费缴纳：01：移动电话 02：固定电话 11：有线和付费电视 17：网络服务费
        便民服务缴费：03：水费 04：电费 05：煤气费 20：物业管理费 22：供暖费 23：废弃物处理费 24：交通出行费用
        政府服务税费缴纳：06：五险一金 13：税收、非税收入
        公益捐款：21：公益捐款
        还款：16：金融机构贷款偿还 27：小额贷款公司贷款偿还
        保险费用缴纳：12：非投资型保险 28：投资型保险
        资金归集：18：资金归集
        小额便民费用缴纳：25：租金 26：会员费
        教育培训：19：教育及培训费用
        基金理财产品购买：14：定期或定额投资产品
        其他取值：保留使用
        */
        agDeductInfoMap.put("agReceiveSubs", "01"); //委托收款款项内容
        agDeductInfoMap.put("agReceivePeriod", "010010"); //委托收款频率 扣款时间单位（ANS2）+扣款次数（ANS4）

        agDeductInfoMap.put("deductionType", "01"); //扣款方式
         // 签约交易时选填字段  maxAmt minAmt
        agDeductInfoMap.put("maxAmt", "1000"); //最高限制金额
        agDeductInfoMap.put("minAmt", "1"); //最低限制金额
        // 当扣款方式为03-委托收款（余额百分比）时，必须填写
        agDeductInfoMap.put("deductionRate", "010"); //扣款金额比例，表示扣款的余额百分比数值，取值范围为001-100

        // 当商户入网配置敏感信息不加密时上送
        bodyMap.put("fwdAcctInfo", Base64.encodeBase64String(JSONObject.wrap(fwdAcctInfoMap).toString().getBytes())); //发起方账户信息域
        bodyMap.put("contractAcctInfo", Base64.encodeBase64String(JSONObject.wrap(contractAcctInfoMap).toString().getBytes())); //签约账户信息域
        bodyMap.put("agDeductInfo", Base64.encodeBase64String(JSONObject.wrap(agDeductInfoMap).toString().getBytes())); //对公委托收款信息域

        // 当商户入网配置敏感信息加密时，在敏感信息域中上送
        // 组装encryptData
        Map<String, Object> encryptDataMap = new HashMap<>();
        encryptDataMap.put("fwdAcctInfo", Base64.encodeBase64String(JSONObject.wrap(fwdAcctInfoMap).toString().getBytes())); //发起方账户信息域
        ...
        encryptDataMap.put("agDeductInfo", Base64.encodeBase64String(JSONObject.wrap(agDeductInfoMap).toString().getBytes())); //对公委托收款信息域
        ...
        bodyMap.put("contractEffectDate", "20230223"); //签约生效日期
        bodyMap.put("contractExpDate", "20240222"); //签约失效日期
    ...
    public static void RsaDemo() {
        ...
        //发起方账户信息域	fwdAcctInfo
        Map<String, Object> fwdAcctInfoMap = new HashMap<>();
        fwdAcctInfoMap.put("bankNo", "111122223333"); //开户行行号
        fwdAcctInfoMap.put("acctNo", "11112222333344445555"); //账号
        fwdAcctInfoMap.put("acctName", "小王"); //账户名称
        //账号类型取值：x1：个人银行卡借记账户 x2：个人银行卡贷记账户 x3：个人银行非卡账户
        // x4：个人银行支付账户 y1：单位银行卡账 y2：单位银行非卡账户 y3：单位支付账户 99：其他
        fwdAcctInfoMap.put("acctType", "x1");
