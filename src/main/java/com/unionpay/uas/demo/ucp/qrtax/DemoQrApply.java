 ...
 */
public class DemoQrApply {

    private static final Logger logger = Logger.getLogger(DemoUAG.class);
    ...
    public static void gm(){
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.qr.bill.apply");       // 交易类型 根据实际业务填写
        ...
        // 组装账单报文
        List billDetailInfoList = new ArrayList();
        Map<String, Object> billMap = new HashMap<>();
        billMap.put("code", "S0_9800_0000");
        billMap.put("tag", "税单一");
        billMap.put("index", "01");
        // 组装账单的form
        Map<String, String> form = new HashMap<>();
        form.put("usr_num", "11000021000149719015");//纳税人识别号
        form.put("col_organ_cd", "21100000000");//征收机关代码
        form.put("col_voucher_no", "131011400002021010");//应征凭证序号
        form.put("proc_flg", "03");//模式
        billMap.put("form", form);

        billDetailInfoList.add(billMap);
        bodyMap.put("billDetailInfo", Base64.encodeBase64String(JSONObject.wrap(billDetailInfoList).toString().getBytes()));

        //申码配置字段
        Map<String, Object> tipsCodeApplyInfoMap = new HashMap<>();
        tipsCodeApplyInfoMap.put("bt","U1");//业务类型
        tipsCodeApplyInfoMap.put("ek","00");//加密密钥索引
        tipsCodeApplyInfoMap.put("ck","00");//地区码
        bodyMap.put("tipsCodeApplyInfo", tipsCodeApplyInfoMap);

        bodyMap.put("reqReserved", "reqReserved"); //请求方保留域
