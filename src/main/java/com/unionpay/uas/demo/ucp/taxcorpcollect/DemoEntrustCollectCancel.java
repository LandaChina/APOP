...
/**
 * 内容网关税费对公代收业务，解约接口demo
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class DemoEntrustCollectCancel {

    private static final Logger logger = Logger.getLogger(DemoEntrustCollectCancel.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.tax.corporate.entrust.collect.cancel");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("bussInfo", "91310114MA1GXBNG2T"); //业务信息.纳税人识别号
        ...
        */
        ...
        bodyMap.put("contractNo", "156"); //签约协议号,选填
        bodyMap.put("countryCode", "156"); //国家代码,当地区代码出现时必填
        bodyMap.put("areaCode", "1100"); //地区代码,选填
        bodyMap.put("remark", "这是附言"); //附言,选填
        bodyMap.put("summary", "这是摘要"); //业务摘要,选填
        bodyMap.put("reqReserved", "这是请求方保留域"); //请求方保留域,选填
    ...
    public static void RsaDemo() {
        ...
        bodyMap.put("orderId", DemoUtil.getOrderId()); //商户订单号，8-32位数字字母，不能含“-”或“_”，可以自行定制规则，此处为演示方便默认取了个值
        bodyMap.put("txnTime", DemoUtil.getTxnTime()); //订单发送时间，格式为yyyyMMddHHmmss，取北京时间，此处为演示方便默认取了个值
        bodyMap.put("bussMerId", "122354646566666"); //不上送征收机关代码时，必填
        bodyMap.put("colOrganCd", "128834799"); //不上送征收机关代码时，必填
        bodyMap.put("bussCode", "J1_9800_XJ03"); //业务代码
        bodyMap.put("bussInfo", "1232424252526"); //业务信息.纳税人识别号
