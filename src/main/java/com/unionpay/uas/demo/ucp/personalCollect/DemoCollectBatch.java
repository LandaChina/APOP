...
/**
 * 内容网关税费个人代收业务，税费代收(批量)（渠道侧->银联）demo
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class DemoCollectBatch {

    private static final Logger logger = Logger.getLogger(DemoCollectBatch.class);
    ...
    public static void gm() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.tax.person.entrust.collect.batch");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("accessType", "");//接入类型,0：普通商户直连接入
        bodyMap.put("merId", "");//商户代码
        bodyMap.put("batchNo", "");//批次号
        bodyMap.put("txnTime", "");//交易发送时间
        bodyMap.put("totalQty", "");//总笔数,对应TPCI文件中所有明细数量之和
        bodyMap.put("totalAmt", "");//总金额,对应TPCI文件中所有明细金额之和
        bodyMap.put("backUrl", "");//后台通知地址
        bodyMap.put("reqReserved", "");//请求方保留域


        //本域取值为对TPCI文件内容采用DEFLATE算法进+行压缩并经过Base64编码后的字符串,明文请根据实际情况填写,仅提供明文处理方法
        String fileContent = "";
        try {
...
//        encDataMap.put("fileContent", fileContent);
//
