...
/**
 * 内容网关税费个人代收业务，税费代收(单笔)（渠道侧->银联）demo
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class DemoCollectPerson {

    private static final Logger logger = Logger.getLogger(DemoCollectPerson.class);
    ...
    public static void gm() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.tax.person.collect");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("currencyCode", "");//交易币种,默认156
        bodyMap.put("billPayType", "");//支付类型,4-后台无卡
        ...
        form2.put("usr_num", "");//纳税人识别号
        form2.put("col_organ_cd", "");//征收机关代码
        form2.put("col_voucher_no", "");//应征凭证序号
        form2.put("proc_flg", "");//模式：03-融合模式

        billMap2.put("form", form2);
        bodyMap.put("billDetailInfo", Base64.encodeBase64String(JSONObject.wrap(billMap2).toString().getBytes()));

        // 组装委托收款信息域，仅提供组装方法的参考，需要根据实际进行修改;委托关系期限和代收频率必填
        Map<String, Object> collectInfo = new HashMap<>();
//        collectInfo.put("maxLimit", "");//最高限额
//        collectInfo.put("minLimit", "");//最低限额
        collectInfo.put("validMonth", "");//委托关系期限
        collectInfo.put("frequency", "");//代收频率
//        collectInfo.put("billPeriod", "");//账单区间
//        collectInfo.put("protocolType", "");//协议类型
//        collectInfo.put("totalReceive", "");//代收扣款总期数
//        collectInfo.put("presentReceive", "");//代收扣款当前期数
        bodyMap.put("collectInfo", collectInfo);//委托收款信息域,委托关系期限和代收频率必填
