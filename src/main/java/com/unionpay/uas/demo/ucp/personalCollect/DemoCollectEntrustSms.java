...
/**
 * 内容网关税费个人代收业务，税费代收签约发送短信验证码（渠道侧->银联）demo
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class DemoCollectEntrustSms {

    private static final Logger logger = Logger.getLogger(DemoCollectEntrustSms.class);
    ...
    public static void gm() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.ucp.tax.person.entrust.collect.smscode");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("orderId", "");//商户订单号,需与后续签约交易的订单号一致
        bodyMap.put("txnTime", "");//交易发送时间,需与后续签约交易的订单号一致
...
//        collectInfo.put("presentReceive", "");//代收扣款当前期数
        ...
        // 组装账户验证信息域，仅提供组装方法的参考，需要根据实际进行修改;账号,手机号必填
...
//        bodyMap.put(SDKConstants.param_encryptData, toBeEncMap); // 如果不需要携带敏感信息并由网关解密, 则注释这行代码
    ...
    public static void rsa() {
        ...
        Map<String, Object> bodyMap = new HashMap<>();
        bodyMap.put("accessType", "");//接入类型
        bodyMap.put("merId", "");//商户代码,需与后续签约交易的订单号一致
        bodyMap.put("orderId", "");//商户订单号,需与后续签约交易的订单号一致
        bodyMap.put("txnTime", "");//交易发送时间,需与后续签约交易的订单号一致
        bodyMap.put("bussMerId", "");//行业商户代码,不上送征收机关代码时，必填
        bodyMap.put("colOrganCd", "");//征收机关代码,不上送行业商户代码时，必填
