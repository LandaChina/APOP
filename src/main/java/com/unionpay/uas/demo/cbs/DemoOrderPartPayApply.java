...
/**
 *  cbs 直连商户 demo-订单批次支付申请
 *  订单批次支付申请交易适用于冻结支付场景中，用于将已冻结资金（部分或全部）支付至收款方账户。账户行收到指令后，先返回指令已受理(应答码为：IS10030000,表示批次支付申请已受理并延迟反馈支付结果)；待资金支付交易处理完毕后再通过“订单批次支付结果通知”交易反馈资金支付结果。
 */
public class DemoOrderPartPayApply {

    private static final Logger logger = Logger.getLogger(DemoOrderPartPayApply.class);
...
//        RsaDemo();
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "pay.b2b.000106");       // 交易类型 根据实际业务填写
        ...
        /*******************原订单交易信息**************************/
        // 原订单交易txnNo
        bodyMap.put("origTxnNo", "1234567890123453TvnNahqi4QZXmQjkhYA");
        // 原订单交易txnDate
        bodyMap.put("origTxnDate", "20220704");
        // 原订单交易
        bodyMap.put("origPlatformOrderNo", "20220704170451000001");
        bodyMap.put("origBizMethod", "pay.b2b.000101");
        bodyMap.put("currencyCode","156");
        bodyMap.put("transferApplyAmt",1);
        bodyMap.put("payeeInfo",buildPayeeInfo() ); //订单发送时间，格式为yyyyMMddHHmmss，取北京时间，此处为演示方便默认取了个值
    ...
    private static Map buildPayeeInfo(){
        ...
        payeeInfo.put("bankName","测试bankName");
