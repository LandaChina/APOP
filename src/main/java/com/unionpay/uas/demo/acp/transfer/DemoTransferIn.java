package com.unionpay.uas.demo.acp.transfer;
...
public class DemoTransferIn {
    private static final Logger logger = Logger.getLogger(DemoTransferIn.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.trade.transfer.in");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("postscript", "");//附言
        bodyMap.put("regionCd","");//地区代码
        bodyMap.put("termId","");//终端号
        bodyMap.put("bankUserId","");//手机银行账户ID
        bodyMap.put("bankAppId","");//手机银行APPID
        bodyMap.put("bankUserRegPhoneNo","");//手机银行APP账户的注册手机号
        bodyMap.put("payAppId","");//支付工具代码

        Map<String, String> merInfo = new HashMap<>();
        merInfo.put("merAbbr","");
        merInfo.put("merName","");
        merInfo.put("merCatCode","");
        bodyMap.put("merInfo",merInfo);//商户信息域

        Map<String, String> creditInfo = new HashMap<>();
        creditInfo.put("payeeBankNo","");//转入方信息域中账户类型accType为10、40时必填
        creditInfo.put("businessLicenseRegNo","");
        bodyMap.put("creditInfo",creditInfo);//贷记信息域


        //风险信息域
        Map<String, String> riskRateInfo = new HashMap<>();
        riskRateInfo.put("shippingFlag","");
        riskRateInfo.put("shippingCountryCode","");
        riskRateInfo.put("shippingProvinceCode","");
        bodyMap.put("riskRateInfo",riskRateInfo);

        //接入方合规信息域
        Map<String, String> accLegalInfo = new HashMap<>();
        accLegalInfo.put("verifyBitmap","");//验证方式
        accLegalInfo.put("verifyTime","");//验证时间

        Map<String, String> transferOutVerifyInfo = new HashMap<>();
        transferOutVerifyInfo.put("accNo","");
        transferOutVerifyInfo.put("customerNm","");
        transferOutVerifyInfo.put("payerBusinessLicenseRegNo","");
//        transferOutVerifyInfo.put("accNum","");//转入方信息域账户类型是10时，与accNo必填其一
        bodyMap.put("transferOutVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(transferOutVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));//转出方信息域


        Map<String, String> transferInVerifyInfo = new HashMap<>();
        transferInVerifyInfo.put("accNo","");
        transferInVerifyInfo.put("customerNm","");
        transferInVerifyInfo.put("accType","");

        bodyMap.put("transferInVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(transferInVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));//转入方信息域

        //转入卡标记化支付信息域
//        Map<String, String> transferInTokenInfo = new HashMap<>();
//        transferInTokenInfo.put("token","");
//        transferInTokenInfo.put("trId","");
//        bodyMap.put("transferInTokenInfo", transferInTokenInfo);

        //转出卡标记化支付信息域
//        Map<String, String> transferOutTokenInfo = new HashMap<>();
//        transferOutTokenInfo.put("token","");
//        transferOutTokenInfo.put("trId","");
//        bodyMap.put("transferOutTokenInfo", transferOutTokenInfo);


        // 需要加密的敏感信息报文 根据实际业务填写,商户入网配置敏感信息加密时使用如下注释代码
//        Map<String, Object> encDataMap = new HashMap<>();
//        encDataMap.put("transferInVerifyInfo", bodyMap.remove("transferInVerifyInfo"));//明文中删除payeeVerifyInfo，并设置到待加密域中
//        encDataMap.put("transferOutVerifyInfo", bodyMap.remove("transferOutVerifyInfo"));//明文中删除transferOutVerifyInfo，并设置到待加密域中
...
//        bodyMap.put(SDKConstants.param_encryptData, toBeEncMap); // 如果不需要携带敏感信息并由网关解密, 则注释这行代码
    ...
    public static void RsaDemo() {
        ...
        transferOutVerifyInfo.put("payerBusinessLicenseRegNo","");
        //transferOutVerifyInfo.put("accNum","");//转入方信息域账户类型是10时，与accNo必填其一
        bodyMap.put("transferOutVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(transferOutVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));//收款方信息域
        ...
        transferInVerifyInfo.put("customerNm","");
        transferInVerifyInfo.put("accType","");
        bodyMap.put("transferInVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(transferInVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));//收款方信息域
