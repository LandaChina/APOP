 ...
 */
public class DemoAcsTrans {

    private static final Logger logger = Logger.getLogger(DemoAcsTrans.class);
    ...
    public static void gm(){
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntFsas.corporation.acs.trans");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("busiAbstract", "");//业务摘要信息
        bodyMap.put("reqReserved", "");//请求方保留域
        bodyMap.put("sourcesOfFunds", "");//资金来源


        Map<String, String> payerVerifyInfo = new HashMap<>();
        payerVerifyInfo.put("payerNm","");//付款方名称
        payerVerifyInfo.put("payerAccNum","");//付款方账户号
        payerVerifyInfo.put("payerAccType","");//付款方账户类型
        payerVerifyInfo.put("payerBusinessLicenseRegNo","");//付款方统一社会信用代码
        ...
        encDataMap.put("payeeVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(payeeVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));
        encDataMap.put("payerVerifyInfo", payerVerifyInfo);
