...
/**
 * 文件传输类交易    bizMethod：upntAcp.trade.file
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class Demo21TradeFile {

    private static final Logger logger = Logger.getLogger(Demo21TradeFile.class);
    ...
    public static void GmDemo() {
        ...
        bodyMap.put("fileType", "00"); //文件类型,依据实际业务情况而定，默认：00
        bodyMap.put("reqReserved", "这是请求方保留域"); //请求方保留域
        // 清算信息域
        // 必送：settleDate-清算日期
        // 可选：settleNo-清算场次
        // 如涉及多场次清算，则上送settleNo信息，并获取settleNo取值的清算场次文件。
        bodyMap.put("settleInfo", buildSettleInfo());    // 商户信息域,根据实际接入方填写
    ...
    private static Map buildSettleInfo(){
        Map<String, Object> settleInfo = new HashMap<String, Object>();
        settleInfo.put("settleDate", ""); //清算日期
        settleInfo.put("settleCurrencyCode", ""); //清算币种
        settleInfo.put("settleAmt", ""); //清算金额
        settleInfo.put("settleKey", ""); //清算主键
        settleInfo.put("settleNo", ""); //清算场
        return settleInfo;
    }
