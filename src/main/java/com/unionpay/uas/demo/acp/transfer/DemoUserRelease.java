...
public class DemoUserRelease {
    private static final Logger logger = Logger.getLogger(DemoUserRelease.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.trade.user.release");       // 交易类型 根据实际业务填写
        ...
        //银行卡验证信息及身份信息
        Map<String, String> accountVerifyInfo = new HashMap<>();
        accountVerifyInfo.put("certifTp","");
        accountVerifyInfo.put("certifId","");
        accountVerifyInfo.put("customerNm","");
        bodyMap.put("accountVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(accountVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));


        // 需要加密的敏感信息报文 根据实际业务填写,商户入网配置敏感信息加密时使用如下注释代码
//        Map<String, Object> encDataMap = new HashMap<>();
//        encDataMap.put("accountVerifyInfo", bodyMap.remove("accountVerifyInfo"));//明文中删除accountVerifyInfo，并设置到待加密域中
