...
public class DemoPayment {
    private static final Logger logger = Logger.getLogger(DemoPayment.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.trade.payment");       // 交易类型 根据实际业务填写
...
//        bodyMap.put("verfPriKey","");//账户验证关联交易主键
//        bodyMap.put("origQryId","");//原交易查询流水号
...
//        bodyMap.put("tokenInfo",);
        ...
        bodyMap.put("merInfo",merInfo);

        //收款方信息域
        Map<String, String> payeeVerifyInfo = new HashMap<>();
        payeeVerifyInfo.put("certifTp","");//证件类型
        payeeVerifyInfo.put("certifId","");//证件号码
        payeeVerifyInfo.put("accNo","");//收款方账号
        payeeVerifyInfo.put("payeeAccType","");//收款方账户类型
        payeeVerifyInfo.put("customerNm","");
        payeeVerifyInfo.put("accType","");
        bodyMap.put("payeeVerifyInfo", Base64.encodeBase64String(JSONObject.wrap(payeeVerifyInfo).toString().getBytes(Charset.forName("UTF-8"))));
...
//        payerVerifyInfo.put("payerAccNum","");//与payerAccNo必送其一
        payerVerifyInfo.put("payerAccType","");
