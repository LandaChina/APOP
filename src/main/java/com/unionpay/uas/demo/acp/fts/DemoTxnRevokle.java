...
public class DemoTxnRevokle {

    private static final Logger logger = Logger.getLogger(DemoTxnRevokle.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "soluFts.fts.cis.txnRevoke");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("origQryId", ""); //原交易时间 若上送原交易商户订单号必须上送原交易时间，yyyyMMHHhhmmss。
        bodyMap.put("origOrderId", DemoUtil.getOrderId()); // 原交易商户订单号 原商户订单号和原查询查询订单号二选一
        bodyMap.put("origTxnTime", DemoUtil.getTxnTime()); //原交易时间 若上送原交易商户订单号必须上送原交易时间，yyyyMMHHhhmmss。
