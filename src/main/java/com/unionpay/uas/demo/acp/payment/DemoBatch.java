...
public class DemoBatch {
    private static final Logger logger = Logger.getLogger(DemoBatch.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.trade.batch");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("batchNo","");//批次号，当天唯一,支持数字与字母，商户号+批次号+交易时间确定一笔交易
        bodyMap.put("totalQty","");//总笔数
        bodyMap.put("totalAmt","");//总金额
        //贷记批量文件内容
        bodyMap.put("batchType","03");//批量交易类型
        String fileContent03="";//贷记的文件内容
        String fileContent = fileContent03;
        try {
            fileContent = deflateEncode(fileContent.getBytes(StandardCharsets.UTF_8));
        } catch (Exception e) {
            logger.info("fileContent压缩失败");
        }
        bodyMap.put("fileContent",fileContent);
        //对公批量文件内容
//        bodyMap.put("batchType","11");//批量交易类型
//        String fileContent11="7.0.0,2422,2,保留域1,保留域2\nmerId,orderId,currencyCode,txnAmt,accType,accNo,customerNm,postscript,busiAbstract,sourcesOfFunds,payerAccNo,payerNm,payerAccType,payerBusinessLicenseRegNo,reqReserved,payeeBankNo,regionCd\nAUTO000000000YL,XXXXZ1620931480326302XCXZX0,156,1211,10,620200123456784709,张三,附言,摘要,02,620200123456784233,张转出,010101,123456789012345678,商户自定义,102100099996,3100\nAUTO000000000YL,XXXXZ1620931480326302XCXZX1,156,1211,10,620200123456784709,张三,附言,摘要,02,620200123456784233,张转出,010101,123456789012345678,商户自定义,102100099996,3100\nEND,2";//对公的文件内容
//        bodyMap.put("fileContent",fileContent11);
//        fileContent = fileContent11;
//        try {
//            fileContent = deflateEncode(fileContent.getBytes(StandardCharsets.UTF_8));
//        } catch (Exception e) {
//            logger.info("fileContent压缩失败");
...
//        encDataMap.put("fileContent", bodyMap.remove("fileContent"));//明文中删除payeeVerifyInfo，并设置到待加密域中
