...
/**
 * 凭据创建申请       bizMethod：upntCbs.emop.bill.create
 * 声明：以下代码只是为了方便商户测试而提供的样例代码，商户可以根据自己需要，按照技术文档编写。该代码仅供参考，不提供编码，性能，规范性等方面的保障<br>
 */
public class Demo04EmopBillCreate {

    private static final Logger logger = Logger.getLogger(Demo04EmopBillCreate.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_bizMethod, "upntCbs.emop.bill.create");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("billName", ""); //凭据名称
        bodyMap.put("billValidBegin", ""); //凭据开始日期
        bodyMap.put("billValidUntil", ""); //凭据结束日期
        bodyMap.put("billAmount", ""); //凭据额度
        bodyMap.put("billMaxTimes", ""); //凭据最大可用次数
        bodyMap.put("billSingleLimit", ""); //凭据单笔限额
        bodyMap.put("usageRangeNo", ""); //应用范围编号
        bodyMap.put("merList", buildMerList()); //商户列表
        ...
        encDataMap.put("billOwnerInfo", buildBillOwnerInfo()); //凭据持有人信息
    ...
    private static List<String> buildMerList() {
    ...
    private static Map buildBillOwnerInfo() {
        Map<String, Object> merInfo = new HashMap<String, Object>();
        merInfo.put("billOwnerNm", ""); //凭据持有人姓名
        merInfo.put("billOwnerMobileNo", ""); //凭据持有人手机号
        merInfo.put("billOwnerCertifTp", ""); //凭据持有人证件类型
        merInfo.put("billOwnerCertifId", ""); //凭据持有人证件号码
        merInfo.put("acctRegInfo", buildAcctRegInfo()); //账户注册信息
        merInfo.put("deviceEnvInfo", buildDeviceEnvInfo()); //设备环境信息
        return merInfo;
    }

    private static Map buildAcctRegInfo() {
        Map<String, Object> acctRegInfo = new HashMap<String, Object>();
        acctRegInfo.put("acctRegDate", ""); //账户注册日期
        acctRegInfo.put("acctRegPhone", ""); //账户注册手机号
        acctRegInfo.put("acctRegMail", ""); //账户注册邮箱地址
        acctRegInfo.put("acctInfUpdTm", ""); //账户关键信息修改时间
        return acctRegInfo;
    }

    private static Map buildDeviceEnvInfo() {
        Map<String, Object> deviceEnvInfo = new HashMap<String, Object>();
        deviceEnvInfo.put("deviceType", ""); //设备类型
        deviceEnvInfo.put("deviceId", ""); //设备标识
        deviceEnvInfo.put("deviceLocation", ""); //设备位置信息
        deviceEnvInfo.put("deviceMode", ""); //设备型号
        deviceEnvInfo.put("deviceNumber", ""); //设备 SIM卡号码
        deviceEnvInfo.put("deviceSIMNumber", ""); //设备SIM卡数量
        deviceEnvInfo.put("sourceIp", ""); //IP地址
        deviceEnvInfo.put("macAddr", ""); //MAC地址
