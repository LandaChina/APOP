 ...
 */
public class DemoUOGGTS24 {

    private static final Logger logger = Logger.getLogger(DemoUOGGTS24.class);
    ...
    public static void gm(){
        ...
        // 以下新增头字段从请求头中取,原样返回即可
        String GTSNb = reqHeader.get(SDKConstants.param_gtsnb);
        String signIssuer = reqHeader.get(SDKConstants.param_signIssuer);

        resData.put(SDKConstants.param_gtsnb, GTSNb);
        resData.put(SDKConstants.param_signIssuer, signIssuer);
    ...
    public static void rsa(){
        ...
        try {
            ...
            if (repBody.containsKey("encryptData")) {
                ...
                String certId = (String) encryptData.get(SDKConstants.param_certId);
                try {
                    String decodeStr = UasService.rsaDecGTS24(decData, decKey, certId);
                    logger.info("解银联加密后的敏感信息的报文：" + decodeStr);
                } catch (Exception e) {
                    logger.error(e);
                }
        ...
        try {
            ...
            String key = DemoUtil.getRandomString(16);
            logger.info("随机生成的对称密钥key: " + key + "\n");
            toBeEncMap = UasService.rsaEncGTS24(toBeEncData, key, "utf-8");
        } catch (Exception e) {
            logger.error(e);
        }
