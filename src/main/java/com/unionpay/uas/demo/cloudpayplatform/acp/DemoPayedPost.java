 ...
 * 云闪付网络支付平台-追缴状态更新接口
 */
public class DemoPayedPost {

    private static final Logger logger = Logger.getLogger(DemoPayedPost.class);
    ...
    public static void GmDemo() {
        ...
        // 组装HTTP请求报文头
        reqData.put(SDKConstants.param_version, "7.1.0");                // 版本号 根据实际业务填写
        ...
        reqData.put(SDKConstants.param_bizMethod, "upntAcp.payed.post");       // 交易类型 根据实际业务填写
        ...
        bodyMap.put("indUsrId", "");//银行用户id,用户授权开通时银行用户侧id
        bodyMap.put("origTn", "");//原交易TN,填写本次追缴状态更新所对应的垫付交易TN，以此字段关联一笔垫付交易
        bodyMap.put("accessType", "");//接入类型
        bodyMap.put("acqInsCode", "");//受理机构代码,接入类型accessType为1收单机构接入时需上送
        Map<String, String> mchntInfoMap = new HashMap<>();
        mchntInfoMap.put("mchntCatCode", "");//商户类别
        mchntInfoMap.put("mchntName", "");//商户名称
        bodyMap.put("mchntInfo", JSONObject.wrap(mchntInfoMap));
        bodyMap.put("merId", "");//商户号,与下面的“订单号”、“订单时间”共同组成交易主键
        bodyMap.put("orderId", "");//订单号,商户订单号
        bodyMap.put("txnTime", "");//订单时间,订单的发送时间
        bodyMap.put("reqReserved", "");//请求方保留域,整体进行Base64编码
