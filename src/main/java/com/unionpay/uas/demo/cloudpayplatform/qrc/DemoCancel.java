...
/**
 * 云闪付网络支付平台-被扫/主扫消费撤销(付款方) 移动应用前置模式
...
public class DemoCancel {
    ...
    private static final Logger logger = Logger.getLogger(DemoCancel.class);
    ...
    public static void gm(Map<String, String> reqHeader, String reqBodyStr) {
        ...
        // 组装HTTP应答报文头
        resData.put(SDKConstants.param_bizMethod, "upntQrc.accCqr.cancel");       // 交易类型 根据实际业务填写,upntQrc.accCqr.cancel（被扫）或 upntQrc.accMqr.cancel（主 扫）
    ...
    public static void rsa(Map<String, String> reqHeader, String reqBodyStr) {
        ...
        // 组装http请求报文头
        resData.put(SDKConstants.param_bizMethod, "upntQrc.accCqr.cancel");       // 交易类型 根据实际业务填写,upntQrc.accCqr.cancel（被扫）或 upntQrc.accMqr.cancel（主扫）
        resData.put(SDKConstants.param_version, "1.0.0");                // 版本号 根据实际业务填写
